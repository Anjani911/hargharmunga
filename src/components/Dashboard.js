...
const RegisterModal = ({ open, onClose }) => {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2000 }}>
      <div style={{ background: "#fff", borderRadius: 16, padding: 32, width: "90%", maxWidth: 600, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 5px 20px rgba(0,0,0,0.25)", position: "relative" }}>
        <button onClick={onClose} style={{ position: "absolute", top: 12, right: 16, border: "none", background: "transparent", fontSize: 28, cursor: "pointer", lineHeight: 1 }}>
          &times;
        </button>
        <AnganwadiAdd />
      </div>
...
const Dashboard = () => {
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const rows = [...table.querySelectorAll("tbody tr")].map((tr) => {
          const obj = {};
          [...tr.querySelectorAll("td")].forEach((td, i) => (obj[cols[i]] = td.textContent.trim()));
          return obj;
        });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f5f5f5", display: "flex" }}>
      {/* ------------- SIDEBAR ------------- */}
      <aside style={{ width: 260, background: "linear-gradient(180deg,#2E7D32,#388e3c)", color: "#fff", display: "flex", flexDirection: "column", padding: "32px 0", boxShadow: "2px 0 12px rgba(44,62,80,0.06)" }}>
        <div style={{ fontWeight: "bold", fontSize: 24, textAlign: "center", marginBottom: 24 }}>
          🌳 हर घर मुंगा
        </div>

        {/* Dashboard selector dropdown */}
        <div style={{ padding: "0 20px", marginBottom: 24 }}>
          <label style={{ color: "#fff", fontSize: 14, marginBottom: 6, display: "block" }}>Choose Dashboard</label>
          <select onChange={handleDashboardSelect} defaultValue="student" style={{ width: "100%", padding: 12, borderRadius: 8 }}>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 32px" }}>
          ...
          <div style={{ background: "#fff", borderRadius: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.08)", padding: 24, marginBottom: 32 }}>
            <form onSubmit={(e) => e.preventDefault()} style={{ display: "flex", gap: 12, alignItems: "center" }}>
              ...
              <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search by name or mobile number" style={{ padding: 12, borderRadius: 8, border: "1px solid #ccc", fontSize: 16, flex: 1 }} />
            ...
            {loading ? (
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 16 }}>
                {filteredStudents.map((s, i) => (
                  <StudentCard key={s.id || i} student={s} onClick={setSelectedStudent} />
                ))}
              </div>
            )}
          </div>
